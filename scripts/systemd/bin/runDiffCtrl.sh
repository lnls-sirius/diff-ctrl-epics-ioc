...
export DIFF_CURRENT_LOW_LIMIT=DIFF_${DIFF_INSTANCE}_LOW_LIMIT
export DIFF_CURRENT_HIGH_LIMIT=DIFF_${DIFF_INSTANCE}_HIGH_LIMIT
export DIFF_CURRENT_DEVICE_TELNET_PORT=DIFF_${DIFF_INSTANCE}_TELNET_PORT
...
export DIFF_LOW_LIMIT=${!DIFF_CURRENT_LOW_LIMIT}
export DIFF_HIGH_LIMIT=${!DIFF_CURRENT_HIGH_LIMIT}
export DIFF_DEVICE_TELNET_PORT=${!DIFF_CURRENT_DEVICE_TELNET_PORT}

if [ -z "${DIFF_CURRENT_DEVICE_TELNET_PORT}" ]; then
    echo "TELNET port is not set." >&2
    exit 1
fi

./runProcServ.sh \
    -t "${DIFF_DEVICE_TELNET_PORT}" \
